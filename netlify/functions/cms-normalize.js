...
function toIsoTime(hourValue, minuteValue, meridiem) {
...
function parseLegacyHoursString(value) {
  const raw = String(value || '').trim();
  if (!raw) return null;
  if (/^closed$/i.test(raw)) return { ...CLOSED_DAY };
...
function normalizeDayHours(value, strictFallback) {
...
function normalizeWeeklyHoursMap(hours, strictHours) {
  return Object.fromEntries(
    dayKeys.map((dayKey) => [
      dayKey,
      normalizeDayHours(hours && hours[dayKey], strictHours && strictHours[`${dayKey}Hours`]),
    ])
  );
}
...
function normalizeSpecialHours(data) {
  const normalized = [];
  const currentSpecialHours = Array.isArray(data && data.specialHours) ? data.specialHours : [];

  currentSpecialHours.forEach((event, index) => {
    const title = String((event && (event.title || event.occasion)) || '').trim() || `Event ${index + 1}`;
    const days = Array.isArray(event && event.days)
      ? event.days
          .map((day) => {
  ...
  const legacyClosedRanges = Array.isArray(data && data.closedRange) ? data.closedRange : [];
  legacyClosedRanges.forEach((range, index) => {
    const title =
      String((range && (range.title || range.occasion)) || '').trim() || `Closed Range ${index + 1}`;
    const dates = enumerateDateRange(range && range.startOfBreak, range && range.endOfBreak);
...
function normalizeRegularHoursRanges(data) {
  const regularHoursRanges = Array.isArray(data && data.regularHoursRanges) ? data.regularHoursRanges : [];
  if (regularHoursRanges.length) {
    return regularHoursRanges.map((range, index) => ({
      title: String((range && range.title) || '').trim() || `Range ${index + 1}`,
      startDate: isIsoDate(range && range.startDate) ? String(range.startDate) : '',
      endDate: isIsoDate(range && range.endDate) ? String(range.endDate) : '',
      hours: normalizeWeeklyHoursMap(range && range.hours, null),
    }));
  }

  const migratedHours = normalizeWeeklyHoursMap(data && data.hours, data && data.regularHoursStrict);
  return [
    {
      title: String(
        (data && data.hours && data.hours.title) ||
          (data && data.regularHoursStrict && data.regularHoursStrict.title) ||
          'Default Hours'
      ).trim(),
...
function normalizePickupSettings(value) {
  const parsed = Number(value && value.maxDaysOut);
  return {
    maxDaysOut: Number.isFinite(parsed) && parsed >= 1 && parsed <= 365 ? Math.floor(parsed) : 30,
  };
}

function normalizePricing(data) {
  if (!data || !Array.isArray(data.pricing)) return Array.isArray(data && data.pricing) ? data.pricing : [];
  const marker = '\n\n#### Highlights\n';
  const toText = (s) =>
    String(s || '')
      .replace(/\*\*([^*]+)\*\*/g, '$1')
...
module.exports = { normalizeCmsData };
